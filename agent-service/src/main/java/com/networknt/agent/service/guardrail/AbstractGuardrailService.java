...
public abstract class AbstractGuardrailService implements GuardrailService {
    ...
    <MethodKey> Optional<com.networknt.agent.guardrail.config.InputGuardrailsConfig> getInputConfig(MethodKey method) {
        return Optional.ofNullable(this.inputGuardrails.get(method)).map(InputGuardrailExecutor::config);
    }

    <MethodKey> Optional<com.networknt.agent.guardrail.config.OutputGuardrailsConfig> getOutputConfig(MethodKey method) {
