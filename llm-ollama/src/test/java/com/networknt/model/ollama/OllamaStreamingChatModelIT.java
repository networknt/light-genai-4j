...
class OllamaStreamingChatModelIT extends AbstractOllamaLanguageModelInfrastructure {
    ...
    void should_generate_valid_json() {
        ...
        com.networknt.agent.model.chat.response.ChatResponse response = handler.get();
        String answer = response.aiMessage().text();
    ...
    void should_handle_timeout(int millis) throws Exception {
        ...
        assertThat(error).isExactlyInstanceOf(com.networknt.agent.exception.TimeoutException.class);
    }

    @ParameterizedTest
    @MethodSource("com.networknt.model.ollama.OllamaChatModelIT#notSupportedContentTypesProvider")
    void should_throw_when_not_supported_content_types_used(List<ContentType> contentTypes) {
        ...
        assertThrows(com.networknt.agent.exception.UnsupportedFeatureException.class, () -> model.chat(List.of(userMessage), handler));
    }

    @ParameterizedTest
    @MethodSource("com.networknt.model.ollama.OllamaChatModelIT#supportedContentTypesProvider")
