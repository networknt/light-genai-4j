...
import com.networknt.agent.data.video.Video;
import java.net.URI;
...
class VideoContentTest {
    ...
    void methods() {
        Video urlVideo =
                Video.builder().url(URI.create("https://example.com/video.mp4")).build();
        ...
        assertThat(videoContent)
                .hasToString("VideoContent { " + "video = Video { "
    ...
    void equals_hash_code() {
        ...
        assertThat(v3).isEqualTo(v3).isEqualTo(v4).hasSameHashCodeAs(v4);
    }
    ...
    void builders() {
        ...
        Video base64video =
                Video.builder().base64Data("dmlkZW8=").mimeType("mimeType").build();
